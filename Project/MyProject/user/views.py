...
def signin(request):
    if request.method=="POST":
        ...
        if x==1:
            ...
            return HttpResponse("<script>alert('Yor are Signed In...');location.href='/user/index/'</script>")
        else:
...
def signup(request):
    if request.method=="POST":
        ...
        if a==0:
          ...
          return HttpResponse("<script>alert('You are registered successfully...');location.href='/user/signup/'</script>")#location.href is used for going the page after refresh
        else:
...
def vgallery(request):
    ...
    mydict={"cdata":cdata,"vdata":vdata}
...
def logout(request):
    ...
    if user:
        ...
        return HttpResponse("<script>alert('You are successfully logout....');location.href='/user/index/'</script>")
...
def booking(request):
    ...
    if user:
        ...
        booknow(userid=user, event_name=title, event_picture=epic, speaker_name=sname, city=city, hotel=hotel,
                speaker_picture=spic, event_date=edate, event_price=price, booking_date=datetime.now().date()).save()
...
def myticket(request):
    ...
    if bid is not None:
        ...
        return HttpResponse("<script>alert('Your ticket has been cancelled...');location.href='/user/myticket/'</script>")
    md={"myticket":ticket}
...
def myprofile(request):
    ...
    if user:
        ...
        if request.method == "POST":
            ...
            return HttpResponse(
                "<script>alert('Your profile has been updated..');location.href='/user/myprofile/'</script>")
    md={"mdata":rdata}
